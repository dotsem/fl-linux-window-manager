    ...
    void *userData)
{
  ...
  FlView *flView = FL_VIEW(gtk_container_get_children(GTK_CONTAINER(gtkWindow))->data);
...
void FLWM::WindowManager::sendMethodCall(
    std::string channelName,
    ...
    FlValue *args)
{
  ...
  if (channel == NULL)
  {
    std::cerr << "The method channel is not found for the given name!" << std::endl;
    return;
  }
...
void FLWM::WindowManager::setMonitor(int monitor_index)
{
  ...
  {
    std::cerr << "FLWM::WindowManager::setMonitor: Window is not valid." << std::endl;
    return;
  }

  GtkWindow *gtk_window = this->window->window;

  // Ensure the window is a layer surface. If not, this call might not have the intended effect
  // or could be an error depending on gtk-layer-shell's behavior.
  // flwm usually calls gtk_layer_init_for_window via convertToLayer or in createWindow.
  if (!gtk_layer_is_layer_window(gtk_window))
  {
    std::cerr << "FLWM::WindowManager::setMonitor: Window is not a layer shell window. Call convertToLayer() first." << std::endl;
    // Optionally, you could try to call gtk_layer_init_for_window(gtk_window); here if desired,
    // but it's better if the Dart side ensures the window is already a layer.
    // gtk_layer_init_for_window(gtk_window); // Uncomment if you want to auto-initialize
  }

  GdkDisplay *display = gtk_widget_get_display(GTK_WIDGET(gtk_window)); // Get display associated with the window
  if (!display)
  {
    std::cerr << "FLWM::WindowManager::setMonitor: Failed to get GDK display for window." << std::endl;
    return;
  }

  if (monitor_index == -1)
  { // Special value to unset or use default
    gtk_layer_set_monitor(gtk_window, NULL);
    std::cout << "FLWM::WindowManager::setMonitor: Unset monitor for window ID " << this->window->id << std::endl;
  }
  else
  {
    int n_monitors = gdk_display_get_n_monitors(display);
    ...
    {
      std::cerr << "FLWM::WindowManager::setMonitor: Invalid monitor index " << monitor_index
                << ". Available monitors: " << n_monitors << std::endl;
      return;
    }
    GdkMonitor *monitor = gdk_display_get_monitor(display, monitor_index);
    if (!monitor)
    {
      std::cerr << "FLWM::WindowManager::setMonitor: Failed to get GdkMonitor for index " << monitor_index << std::endl;
      return;
    }
    gtk_layer_set_monitor(gtk_window, monitor);
    std::cout << "FLWM::WindowManager::setMonitor: Set monitor " << monitor_index
              << " for window ID " << this->window->id << std::endl;
  }
}